  ...
  <div class="line">
    ...
    <div class="posterblock" style="">
      <div class="top">
        ...
        <a href="{{route('anime', ['id' => $animeelement->id])}}" class="info-max">
      ...
      <div class="posteblock__genres" style="display: flex; flex-direction: row; color: gray;">
        ...
        <?php
                        $maxGenresToShow = 4; // Ограничение на количество жанров для отображения
                        $genreCount = count($animeelement->genres);
                        $genres = $animeelement->genres->slice(0, $maxGenresToShow); // Выбираем первые $maxGenresToShow жанров

                        $genresString = $genres->implode('Genre', ', '); // Объединяем жанры в строку 

                        if ($genreCount > $maxGenresToShow) {
                            $genresString .= '...'; // Если жанров больше чем мы отображаем, добавляем троеточие
                        }
                    ?>
                    @endforeach
